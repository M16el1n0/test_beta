...
// ===== ЗАДАНИЯ =====
const TASKS = {
    1: { name:'Пополнить баланс', desc:'Пополнить >100 монет',        target:1, reward:15,  rewardType:'gold',   type:'deposit100'  },
    2: { name:'Мины',             desc:'Сыграть в мины 5 раз',        target:5, reward:10,  rewardType:'silver', type:'minesPlayed' },
    3: { name:'Ракетка',          desc:'Сыграть в ракетку 5 раз',     target:5, reward:10,  rewardType:'silver', type:'rocketPlayed'},
    4: { name:'Кейсы',            desc:'Открыть платный кейс 5 раз',  target:5, reward:10,  rewardType:'silver', type:'casesOpened' },
};
...
function updateTasks() {
    ...
    for (let i = 1; i <= 4; i++) { if (userData.tasks[i]) { completed++; totalRewards += TASKS[i].reward; } }
    const tc = document.getElementById('tasks-completed');
    const tr = document.getElementById('total-rewards');
    if (tc) tc.textContent = completed;
    if (tr) tr.textContent = totalRewards;
}
...
function claimTaskReward(id) {
    ...
    if (task.rewardType === 'gold') {
        userData.balance.gold = (userData.balance.gold || 0) + task.reward;
    } else {
        userData.balance.silver += task.reward;
    }
    saveUserData();
    updateBalance();
    updateTasks();
...
function confirmOpenCase() {
    ...
    if (!cfg.free && !cfg.strike) {
        ...
        userData.taskProgress = userData.taskProgress||{};
        userData.taskProgress.casesOpened = (userData.taskProgress.casesOpened||0)+1;
...
function updateInventory() {
    ...
    function renderMcGrid(gridId, items) {
        ...
        grid.innerHTML = '';
        if (!items.length) {
            grid.innerHTML = '<div style="grid-column:span 3;text-align:center;color:#333;padding:32px 0;font-size:0.85rem;">Пусто</div>';
            return;
        }
        items.forEach(g => {
            ...
            card.className = 'inv-card-prf' + (isSold ? ' inv-card-sold' : '');
            card.innerHTML =
                '<div class="inv-card-emoji">' + icon + '</div>' +
                '<div class="inv-card-name">'  + (g.name||'Подарок') + '</div>' +
                '<div class="inv-card-val">'   + (g.value||0) + ' F</div>' +
                (isSold
                    ? '<div class="inv-card-sub" style="color:#333">продано</div>'
                    : '<div class="inv-card-sub">продать</div>');
...
function showInventoryTab(tab) {
    ...
    document.querySelectorAll('.inv-tab-prf, .mc-inv-tab').forEach(btn => btn.classList.remove('active'));
...
function trackDeposit(coins) {
    if (coins > 100) { userData.taskProgress = userData.taskProgress||{}; userData.taskProgress.deposit100 = 1; saveUserData(); updateTasks(); }
}
