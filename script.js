...
function createGameBoard() {
    ...
    for (let i = 0; i < gameState.totalCells; i++) {
        ...
        cell.innerHTML = '<span class="cell-icon">F</span>';
    ...
    function renderMcGrid(gridId, items) {
        ...
        for (let i=0; i<total; i++) {
            ...
            if (i < items.length) {
                ...
                slot.innerHTML = `<span class="mc-slot-icon">${icon}</span><span class="mc-slot-qty" style="position:absolute;bottom:4px;right:6px;font-size:0.6rem;color:rgba(255,255,255,0.7);font-weight:800;">${(g.value||0)}F</span>`;
...
// ===== ПОДАРКИ =====
const GIFT_EMOJIS={bear:'🧸',heart:'❤️',rose:'🌹',gift:'🎁',cake:'🎂',champagne:'🥂',bouquet:'💐',cup:'🏆',ring:'💍',diamond:'💎',crown:'👑'};
function showGiftChoiceModal(gift, winAmount) {
    ...
    setText('new-gift-tier',  winAmount>=1000?'👑 Легендарный':winAmount>=500?'💜 Редкий':winAmount>=100?'🔵 Необычный':'⚪ Обычный');
    // кнопка продать — полная сумма выигрыша
    const sellEl = document.getElementById('sell-amount');
    if (sellEl) sellEl.textContent = winAmount;
    // full-value для кнопки "оставить"
    const fullValEl = document.getElementById('full-value');
    if (fullValEl) fullValEl.textContent = winAmount;
    // кнопка забрать — остаток (выигрыш - цена подарка)
    const giftCost = gift.minValue || 0;
    const remainder = Math.max(0, winAmount - giftCost);
    const keepLbl = document.getElementById('keep-coins-label');
    if (keepLbl) keepLbl.textContent = remainder > 0 ? `+${remainder} F на баланс` : 'без доп. монет';
...
function keepGift() {
    ...
    const msg = remainder > 0
        ? '🎁 Подарок в инвентаре! +' + remainder + ' F на баланс'
        : '🎁 Подарок добавлен в инвентарь!';
    if (typeof showNotif === 'function') showNotif(msg, '#7b5cff');
    else alert(msg);
}

function sellGift() {
    if (!currentNewGift) return;
    const sellPrice = currentNewGift.value || 0;   // полная сумма выигрыша
    ...
    else alert('Продано за ' + sellPrice + 'F');
}

function showManageGiftModal(giftId) {
    const gift = (userData.inventory||[]).find(g => g.id === giftId);
    if (!gift) return;
    const GIFT_EMOJIS_M = {bear:'🧸',heart:'❤️',rose:'🌹',gift:'🎁',cake:'🎂',champagne:'🥂',bouquet:'💐',cup:'🏆',ring:'💍',diamond:'💎',crown:'👑'};
    const emoji = GIFT_EMOJIS_M[gift.type] || '🎁';
    const sellPrice = gift.value || 0; // ПОЛНАЯ цена
    const receivedDate = new Date(gift.receivedDate);
    const unlockDate = new Date(receivedDate.getTime() + 21*24*60*60*1000);
    const now = Date.now();
    const canWithdraw = now >= unlockDate.getTime();
    const daysLeft = Math.max(0, Math.ceil((unlockDate - now) / (24*60*60*1000)));
    const tierLabel = sellPrice>=1000?'👑 Легендарный':sellPrice>=500?'💜 Редкий':sellPrice>=100?'🔵 Необычный':'⚪ Обычный';

    const infoEl = $id('manage-gift-info');
    if (infoEl) {
        infoEl.innerHTML = `
            <div class="manage-gift-display">
                <div class="manage-gift-emoji">${emoji}</div>
                <div class="manage-gift-details">
                    <div class="manage-gift-name">${gift.name||'Подарок'}</div>
                    <div class="manage-gift-tier">${tierLabel}</div>
                    <div class="manage-gift-price">${sellPrice} F</div>
                </div>
            </div>
            <div class="manage-gift-status">
                ${canWithdraw
                    ? '<span class="manage-ready">✅ Готов к выводу</span>'
                    : `<span class="manage-locked">🔒 Заблокирован ещё ${daysLeft} дн.</span>`}
            </div>`;
    }
    const actEl = $id('manage-gift-actions');
    if (actEl) {
        actEl.innerHTML = `
            <button class="manage-btn manage-sell-btn" onclick="confirmSellInventoryGift(${giftId})">
                💰 Продать за ${sellPrice} F
            </button>
            ${canWithdraw ? `<button class="manage-btn manage-withdraw-btn" onclick="withdrawGift(${giftId})">📤 Вывести в Telegram</button>` : ''}
            <button class="manage-btn manage-cancel-btn" onclick="closeManageGiftModal()">Отмена</button>`;
    }
    const modal = $id('manage-gift-modal');
    if (modal) modal.style.display = 'flex';
}

function confirmSellInventoryGift(giftId) {
    const gift = (userData.inventory||[]).find(g => g.id === giftId);
    if (!gift) return;
...
function withdrawGift(giftId) {
    ...
    if (!gift) return;
    closeManageGiftModal();
    if (typeof showNotif === 'function') showNotif('📤 Запрос на вывод отправлен!', '#7b5cff');
}
