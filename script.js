...
async function buyStarPackage(stars, coins) {
    ...
    try {
        ...
        // Fallback: открываем бота для оплаты через команду /topup
        showNotif('⚠️ Открываем бота для оплаты…', '#f59e0b');
        setTimeout(() => {
            if (tg) tg.openTelegramLink('https://t.me/fleep_gift_bot?start=topup_' + stars);
        }, 1000);
    }
}
