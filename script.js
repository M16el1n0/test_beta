...
async function buyStarPackage(stars, coins) {
    if (!tg) {
        // Вне Telegram — просто показываем инструкцию
        alert(`Открой бота @${BOT_USERNAME} и введи:\n/topup ${stars}`);
        return;
    }

    const promo = activePromo?.code || null;
    // Формируем deeplink с параметрами: бот получит /start topup_{stars}_{promo}
    const startParam = promo
        ? `topup_${stars}_${promo}`
        : `topup_${stars}`;

    closeTopUpModal();

    // Открываем бота с нужной командой прямо внутри Telegram
    tg.openTelegramLink(`https://t.me/${BOT_USERNAME}?start=${startParam}`);

    // Показываем инструкцию
    showNotif(`⭐ Открываю бота для оплаты ${stars} звёзд…`, '#8b5cf6');
}
