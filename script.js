...
function createGameBoard() {
    ...
    grid.className = 'mines-grid-new' + (gameConfig.size === 10 ? ' grid-10' : gameConfig.size === 5 ? ' grid-5' : '');
    grid.style.fontSize = '';
    gameState.gameBoard = [];
...
function revealCell(index) {
    ...
    if (cell.isMine) {
        ...
        // Пересчитываем коэффициент динамически
        const totalCells = gameState.totalCells;
        const mines = gameConfig.mines;
        const revealed = gameState.revealedCells;
        // Формула: (totalCells / safeCells) ^ revealed * house_edge
        // Каждый ход умножает на (total - i) / (safe - i)
        let coef = 1.0;
        for (let i = 0; i < revealed; i++) {
            coef *= (totalCells - i) / (totalCells - mines - i);
        }
        coef *= 0.77; // house edge + 20% меньше
        gameState.currentCoefficient = Math.max(1.01, coef);
...
function updateGameInterface() {
    const coef = gameState.currentCoefficient;
    const win = Math.floor(gameState.currentBet * coef);
    setText('current-coef', coef.toFixed(2) + '×');
    setText('current-win', win + ' F');
