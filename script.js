...
function buyStarPackage(stars, coins) {
    ...
    // Отправляем запрос боту — он создаст инвойс и пришлёт в чат
    // После оплаты бот начислит коины через successful_payment
    ...
    try {
        tg.sendData(payload);
        // WebApp закрывается — пользователь увидит инвойс в чате с ботом
        closeTopUpModal();
        tg.close();
    } catch (err) {
        console.error("sendData error:", err);
        if (btn && origHTML) btn.innerHTML = origHTML;
        alert("Ошибка. Убедись что приложение открыто через бота.");
    }
}
