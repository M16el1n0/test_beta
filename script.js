...
function revealCell(index) {
    ...
    if (cell.isMine) {
        ...
        const icon = cell.element.querySelector('.cell-icon');
        if (icon) { icon.textContent = '💣'; icon.style.animation = 'bombPop 0.3s ease'; }
        endGameLose();
    } else {
        cell.element.classList.add('safe');
        const icon = cell.element.querySelector('.cell-icon');
        if (icon) { icon.textContent = '💎'; icon.style.animation = 'gemPop 0.25s ease'; }
...
function updateProfileGifts() {
    var EMOJIS = {bear:'🧸',heart:'❤️',rose:'🌹',gift:'🎁',cake:'🎂',champagne:'🥂',bouquet:'💐',cup:'🏆',ring:'💍',diamond:'💎',crown:'👑',rocket:'🚀',star:'⭐'};
    ...
    var inv = (userData.inventory || []).filter(function(g){ return g.status === 'active' || g.status === 'withdrawn'; });
    ...
    grid.innerHTML = items.map(function(g) {
        var emoji = EMOJIS[g.type] || '🎁';
        var isWithdrawn = g.status === 'withdrawn';
        var opacity = isWithdrawn ? 'opacity:0.5;' : '';
        return '<div class="prf-gift-mini" onclick="showSection("inventory")" style="' + opacity + '">' +
            '<span class="prf-gift-mini-emoji">' + emoji + '</span>' +
            '<div class="prf-gift-mini-val">🟡 ' + (g.value || 0) + '</div>' +
            '<div class="prf-gift-mini-lbl">' + (isWithdrawn ? 'Выведен' : (g.name || 'Подарок')) + '</div>' +
            '</div>';
    }).join('');
}
    ...
    function renderMcGrid(gridId, items) {
        ...
        items.forEach(g => {
            ...
            card.innerHTML =
                ...
                '<span class="inv-gift-emoji">'+icon+'</span>'+
