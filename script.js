...
function updateProfileGifts() {
    ...
    items.forEach(function(g) {
        ...
        iconDiv.innerHTML = giftIcon(g.type, 52);

        var valDiv = document.createElement('div');
        valDiv.className = 'prf-gift-mini-val';
        valDiv.textContent = '\u{1F7E1} ' + (g.value || 0);

        var lblDiv = document.createElement('div');
        lblDiv.className = 'prf-gift-mini-lbl';
        lblDiv.textContent = isW ? '\u0412\u044b\u0432\u0435\u0434\u0435\u043d'
                                 : (g.name || '\u041f\u043e\u0434\u0430\u0440\u043e\u043a');
...
// champagne/crown — только эмодзи, остальные — картинки
function giftIcon(type, size) {
    ...
    return '<span style="font-size:' + Math.round(size * 0.55) + 'px;line-height:1">' + (GIFT_EMOJIS[type] || '\uD83C\uDF81') + '</span>';
}
function showGiftChoiceModal(gift, winAmount) {
    ...
    if (iconEl) {
        iconEl.innerHTML = giftIcon(gift.type, 90);
    }
