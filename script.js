...
function revealCell(index) {
    ...
    if (cell.isMine) {
        ...
        // Формула: произведение вероятностей каждого безопасного хода
        let coef = 1.0;
        for (let i = 0; i < revealed; i++) {
            const remaining = totalCells - i;
            const safeProbability = (remaining - mines) / remaining;
            coef = coef / safeProbability;
        }
        // Небольшой house edge 3%
        coef = coef * 0.97;
        gameState.currentCoefficient = Math.max(1.0, coef);
...
function updateGameInterface() {
    ...
    setText('current-win', gameState.currentBet + ' × ' + coef.toFixed(2) + ' = ' + win + ' F');
