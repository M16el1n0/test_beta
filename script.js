...
// ===== СИНХРОНИЗАЦИЯ ЗОЛОТА С СЕРВЕРА =====
// ⚠️ Замени на свой URL после деплоя на Railway/Render
...
async function syncGoldFromServer() {
    try {
        ...
        if (serverGold !== userData.balance.gold) {
            ...
            saveUserData();
            updateBalance();
            if (serverGold > 0) showGoldSyncNotif(serverGold);
        }
    } catch(e) {
        // Сервер не доступен — используем локальный баланс
        console.log('syncGold: using local balance');
    }
}
