...
function showGoldSyncNotif(gold) {
    ...
    notif.style.cssText = `
        ...
        color:#000;padding:12px 22px;border-radius:16px;
        font-weight:800;font-size:0.95rem;z-index:9999;
        box-shadow:0 8px 30px rgba(245,158,11,0.5);
        text-align:center;
    `;
    notif.innerHTML = `🟡 Баланс обновлён: ${gold} коинов`;
...
function startGame() {
    ...
    gameState.baseCoefficient = gameState.currentCoefficient || 1.5;
    gameState.currentCoefficient = gameState.baseCoefficient;
...
function revealCell(index) {
    ...
    if (cell.isMine) {
        ...
        // Коэффициент растёт с каждой открытой клеткой
        const safeCellsTotal = gameState.totalCells - gameConfig.mines;
        const baseCoef = gameState.baseCoefficient || 1.0;
        const growFactor = 1 + (gameConfig.mines / gameState.totalCells) * 0.8;
        gameState.currentCoefficient = parseFloat((baseCoef * Math.pow(growFactor, gameState.revealedCells)).toFixed(2));
        updateGameInterface();
...
function updateProfileInfo() {
    const tgUser = getTgUser();
    const name = tgUser ? (tgUser.username ? tgUser.username.toUpperCase() : tgUser.first_name.toUpperCase()) : 'ИГРОК';
    
    // Шапка
    const usernameEl = $id('profile-username');
    if (usernameEl) usernameEl.textContent = name;

    // Фото из Telegram
    if (tgUser && tgUser.photo_url) {
        const img = $id('profile-tg-photo');
        const emoji = $id('profile-avatar-emoji');
        if (img) { img.src = tgUser.photo_url; img.style.display = 'block'; }
        if (emoji) emoji.style.display = 'none';
    }

    // Баланс
    const goldBal = $id('profile-gold-bal');
    const silverBal = $id('profile-silver-bal');
    if (goldBal)   goldBal.textContent   = userData.balance.gold   || 0;
    if (silverBal) silverBal.textContent = userData.balance.silver || 0;

    // Статистика
    const tw  = $id('profile-total-won2');
    const twg = $id('profile-total-won-gold');
    const gp  = $id('profile-games-played2');
    const gw  = $id('profile-games-won2');
    if (tw)  tw.textContent  = userData.stats.totalWon    || 0;
    if (twg) twg.textContent = userData.balance.gold      || 0;
    if (gp)  gp.textContent  = userData.stats.gamesPlayed || 0;
    if (gw)  gw.textContent  = userData.stats.gamesWon    || 0;

    // История в профиле
    const histEl = $id('profile-history-list');
    if (histEl) {
        histEl.innerHTML = '';
        const history = (userData.gameHistory || []).slice(0, 20);
        if (!history.length) {
            histEl.innerHTML = '<div style="text-align:center;color:#555;padding:30px;">Нет истории игр</div>';
        } else {
            history.forEach(g => {
                const d = new Date(g.timestamp || Date.now());
                const time = String(d.getHours()).padStart(2,'0') + ':' + String(d.getMinutes()).padStart(2,'0');
                const item = document.createElement('div');
                item.style.cssText = 'background:#13131f;border-radius:12px;padding:12px 14px;display:flex;justify-content:space-between;align-items:center;border:1px solid rgba(255,255,255,0.05);';
                const color = g.isWin ? '#4ade80' : '#f87171';
                const sign  = g.isWin ? '+' : '-';
                item.innerHTML = '<div style="font-size:0.8rem;color:#666;">' + time + '</div>'
                    + '<div style="font-size:0.85rem;color:#aaa;">Ставка: ' + g.bet + 'F</div>'
                    + '<div style="font-weight:800;color:' + color + ';">' + sign + (g.isWin ? g.win : g.bet) + 'F</div>'
                    + '<div style="font-size:0.8rem;color:#888;">×' + (g.coefficient||0).toFixed(2) + '</div>';
                histEl.appendChild(item);
            });
        }
    }

    // Старые элементы если есть
    setText('user-name',        name);
    setText('reg-date',         new Date(userData.createdAt || Date.now()).toLocaleDateString('ru'));
    setText('last-visit',       new Date(userData.lastVisit || Date.now()).toLocaleDateString('ru'));
    setText('user-gold-flip',   userData.balance.gold   || 0);
    setText('user-silver-flip', userData.balance.silver || 0);
}

function switchProfileTab(tab) {
    ['stats','refs','history'].forEach(t => {
        const btn = $id('ptab-' + t);
        const content = $id('ptab-' + t + '-content');
        const active = t === tab;
        if (btn) {
            btn.style.color = active ? '#7b5cff' : '#555';
            btn.style.borderBottom = active ? '2px solid #7b5cff' : '2px solid transparent';
        }
        if (content) content.style.display = active ? 'block' : 'none';
    });
    if (tab === 'history') updateProfileInfo();
}

function shareRefLink() {
    const tgUser = getTgUser();
    const userId = tgUser ? tgUser.id : 'unknown';
    const link = 'https://t.me/fleep_gift_bot?start=ref_' + userId;
...
// ===== КЕЙСЫ — КОНФИГ =====
const CASE_CONFIG = {
    peace:    { name: 'Покой в богатстве', currency: 'silver', cost: 555,  allowGold: true, goldCost: 55 },
...
const CASE_ODDS = [
    { label: 'Ракета 🚀',     value: 50,  pct: '15%', color: '#a78bfa' },
    { label: 'Сердце ❤️',     value: 15,  pct: '23%', color: '#f87171' },
    { label: 'Мишка 🐻',      value: 15,  pct: '23%', color: '#fbbf24' },
    { label: 'Алмаз 💎',      value: 50,  pct: '15%', color: '#38bdf8' },
    { label: 'Шампанское 🍾', value: 50,  pct: '15%', color: '#34d399' },
    { label: 'Кубок 🏆',      value: 100, pct: '9%',  color: '#fcd34d' },
];
...
function selectCase(type) {
    ...
    if (priceEl) {
        ...
        else if (cfg.strike) priceEl.textContent = '7 дней подряд депозита';
    ...
    if (cfg.free || cfg.strike) {
        ...
        // звёздные кейсы: только золото, скрываем переключатель
        if (currDiv) currDiv.style.display = 'none';
    ...
    if (oddsList) {
        ...
        CASE_ODDS.forEach(o => {
            ...
            row.style.cssText = 'display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,0.04);border-radius:10px;padding:8px 12px;';
            row.innerHTML = '<span style="font-size:0.9rem;color:#ddd;">' + o.label + '</span>'
                + '<div style="display:flex;align-items:center;gap:12px;">'
                + '<span style="font-size:0.75rem;color:#666;">' + o.value + ' F</span>'
                + '<span style="font-size:0.85rem;font-weight:800;color:' + o.color + ';">' + o.pct + '</span>'
                + '</div>';
...
// ⚠️ После деплоя на Railway/Render — вставь сюда свой URL
const BACKEND_URL = "https://ВАШ-БОТ.railway.app";
...
async function buyStarPackage(stars, coins) {
    ...
    try {
        // Отправляем initData для верификации пользователя на сервере
        const resp = await fetch(`${BACKEND_URL}/create-invoice`, {
            ...
            body: JSON.stringify({
                ...
                init_data: tg.initData   // подпись Telegram — проверяется на сервере
            })
        });
        ...
        // Открываем нативное окно оплаты Telegram прямо внутри WebApp
        tg.openInvoice(data.invoice_link, (status) => {
            ...
            // "cancelled" — пользователь просто закрыл, ничего не делаем
        });

    } catch (err) {
        console.error("buyStarPackage error:", err);
        alert(`Ошибка: ${err.message}`);
    } finally {
        if (btn && origHTML) { btn.innerHTML = origHTML; btn.style.pointerEvents = ""; }
    }
}
