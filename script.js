...
if (tg) {
    tg.ready();
    tg.expand();
    // Форсируем тёмный фон — работает на всех версиях Telegram
    try { tg.setHeaderColor('#000000'); } catch(e) {}
    try { tg.setBackgroundColor('#000000'); } catch(e) {}
    // Принудительно переопределяем фон через DOM
    document.documentElement.style.setProperty('background', '#000', 'important');
    document.body.style.setProperty('background', '#000', 'important');
    document.body.style.setProperty('background-color', '#000', 'important');
    // Если colorScheme светлая — форсируем тёмную через CSS vars
    if (tg.colorScheme === 'light') {
        document.documentElement.setAttribute('data-theme', 'dark');
        document.documentElement.style.cssText += ';--tg-theme-bg-color:#000000!important;--tg-theme-secondary-bg-color:#0d0d1a!important;';
    }
}
function getTgUser() {
...
async function buyUsdtPackage(coins, usdt) {
    ...
    try {
        ...
        // Сразу открываем CryptoBot
        if (data.wallet) openUsdtLink(data.wallet);
