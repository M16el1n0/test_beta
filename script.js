...
function renderStarPackages() {
    ...
    STAR_PACKAGES.forEach(pkg => {
        ...
        `;
        el.onmouseenter = () => el.style.borderColor = '#7b5cff';
        el.onmouseleave = () => el.style.borderColor = 'rgba(123,92,255,0.3)';
        el.innerHTML = `
            ${hasBonus ? `<div style="position:absolute;top:-8px;right:-4px;background:#f59e0b;color:#000;font-size:0.6rem;font-weight:900;padding:2px 6px;border-radius:20px;">+${Math.round(bonus*100)}%</div>` : ''}
            <div style="font-size:1.4rem;">⭐</div>
            <div style="color:#fff;font-weight:800;font-size:1rem;">${pkg.stars}</div>
            <div style="color:#aaa;font-size:0.7rem;margin:2px 0;">звёзд</div>
            <div style="color:#fcd34d;font-weight:900;font-size:0.95rem;">${finalCoins} 🟡</div>
            ${hasBonus ? `<div style="color:#aaa;text-decoration:line-through;font-size:0.7rem;">${pkg.coins}</div>` : ''}
        `;
...
function buyStarPackage(stars, coins) {
    // Проверяем Telegram WebApp
    if (tg && tg.openInvoice) {
        // ===== НАСТОЯЩИЙ ПЛАТЁЖ ЧЕРЕЗ TELEGRAM STARS =====
        // Бэкенд должен создать инвойс и вернуть ссылку
        // Пример: fetch('/api/create-invoice', { method:'POST', body: JSON.stringify({ stars, coins, userId: tg.initDataUnsafe.user?.id, promo: activePromo?.code }) })
        //   .then(r => r.json()).then(data => tg.openInvoice(data.invoiceLink, callback))
        
        // ЗАГЛУШКА (пока бэкенд не подключён):
        showStarsPaymentPlaceholder(stars, coins);
    } else {
...
function showStarsPaymentPlaceholder(stars, coins) {
    // Временная заглушка — в продакшне здесь будет tg.openInvoice()
    const confirm = window.confirm(
        `⭐ Оплата: ${stars} звёзд Telegram\n🟡 Получите: ${coins} золотых коинов\n\n(Тестовый режим — реальная оплата через бэкенд)`
    );
    if (confirm) {
