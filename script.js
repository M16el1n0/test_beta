...
function cashOut() {
    ...
    if (win >= 15) {
        ...
        else setTimeout(newGame, 1500);
    } else if (win > 0) {
        setTimeout(() => showSmallWinModal(win), 600);
...
function showSmallWinModal(amount) {
    var el = document.getElementById('small-win-amount');
    if (el) el.textContent = amount;
    var modal = document.getElementById('small-win-modal');
    if (modal) modal.style.display = 'flex';
}
function closeSmallWinModal() {
    var modal = document.getElementById('small-win-modal');
    if (modal) modal.style.display = 'none';
}
