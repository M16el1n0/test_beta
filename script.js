...
// ═══ ПОПОЛНЕНИЕ ЧЕРЕЗ БОТА (без бэкенда) ═══
// Пользователь жмёт пакет → открывается бот с командой /topup {stars}
// Бот сам создаёт инвойс и отправляет его в чат
// После оплаты бот отправляет deeplink → startapp=gold_{balance}
// WebApp читает start_param и начисляет монеты

const BOT_USERNAME = 'fleep_gift_bot';

// URL бэкенда на Railway (задаётся здесь один раз)
const BACKEND_URL = 'https://web-production-42c21.up.railway.app';

async function buyStarPackage(stars, coins) {
    if (!tg) {
        alert(`Открой бота @${BOT_USERNAME} и введи:\n/topup ${stars}`);
        return;
    }

    const promo = activePromo?.code || null;

    // Получаем init_data для верификации на сервере
    const initData = tg.initData || '';

    // Запрашиваем invoice link у нашего бэкенда
    let invoiceUrl;
    try {
        ...
        showNotif(`❌ ${e.message}`, '#ef4444');
        return;
    }

    closeTopUpModal();

    // Открываем нативный платёжный экран прямо внутри WebApp
    tg.openInvoice(invoiceUrl, (status) => {
        if (status === 'paid') {
            creditCoins(coins, stars);
        } else if (status === 'cancelled') {
            showNotif('❌ Оплата отменена', '#ef4444');
        } else if (status === 'failed') {
            showNotif('❌ Ошибка оплаты', '#ef4444');
        }
    });
}
