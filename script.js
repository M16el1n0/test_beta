...
function updateProfileGifts() {
    var EMOJIS = {bear:"\u{1F9F8}",heart:"\u2764\uFE0F",rose:"\u{1F339}",gift:"\u{1F381}",cake:"\u{1F382}",champagne:"\u{1F37E}",bouquet:"\u{1F490}",cup:"\u{1F3C6}",ring:"\u{1F48D}",diamond:"\u{1F48E}",crown:"\u{1F451}",rocket:"\u{1F680}",star:"\u2B50"};
    ...
    grid.innerHTML = items.map(function(g) {
        var emoji = EMOJIS[g.type] || '\u{1F381}';
        var isW   = g.status === 'withdrawn';
        var op    = isW ? 'opacity:0.5;' : '';
        var card  = document.createElement('div');
        card.className = 'prf-gift-mini';
        card.setAttribute('style', op);
        card.onclick = function() { showSection('inventory'); };
        card.innerHTML = '<span class="prf-gift-mini-emoji">' + emoji + '</span>'
            + '<div class="prf-gift-mini-val">\uD83D\uDFE1 ' + (g.value || 0) + '</div>'
            + '<div class="prf-gift-mini-lbl">' + (isW ? '\u0412\u044b\u0432\u0435\u0434\u0435\u043d' : (g.name || '\u041f\u043e\u0434\u0430\u0440\u043e\u043a')) + '</div>';
