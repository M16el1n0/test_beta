...
function syncGoldFromCloud() {
    ...
    tg.CloudStorage.getItem('gold_coins', (err, value) => {
        if (err || value === null || value === undefined || value === '') return;
        const cloudGold = parseInt(value) || 0;
        if (cloudGold > userData.balance.gold) {
            // Берём максимум — чтобы не затереть локальные начисления
