...
const GIFT_SYSTEM = {
    ...
    getRandomGift(winAmount) {
        // Подарки чей диапазон содержит winAmount
        const inRange = this.gifts.filter(g => winAmount >= g.minValue && winAmount <= g.maxValue);
        if (inRange.length) return inRange[Math.floor(Math.random() * inRange.length)];
        const below = this.gifts.filter(g => winAmount >= g.minValue);
...
function cashOutRocket() {
    ...
    // Сразу останавливаем ракету и показываем результат
    const cashBtn = $id('rocket-cashout-btn');
    ...
    // Небольшая пауза — потом краш-анимация и новый раунд
    setTimeout(() => {
        crashAnimateRocket();
        startRocketCountdown();
    }, 600);
...
function startRocketCountdown() {
    // Таймер убран — кнопка сразу активна
    const playBtn = $id('rocket-play-btn');
    if (playBtn) {
        playBtn.style.display = 'block';
        playBtn.disabled = false;
...
function claimCasePrize() {
    ...
    const giftObj = GIFT_SYSTEM.getRandomGift(val) || { type: "gift", name: prizeName };
    pendingCasePrize = null;
    pendingCaseType  = null;
    setTimeout(() => showGiftChoiceModal(giftObj, val), 300);
}
...
// ===== ПОДАРКИ =====
const GIFT_EMOJIS = {
    bear: '🧸', heart: '❤️', rose: '🌹', gift: '🎁',
    cake: '🎂', champagne: '🥂', bouquet: '💐', cup: '🏆',
    ring: '💍', diamond: '💎', crown: '👑'
};

function getGiftTier(val) {
    if (val >= 2000) return '👑 Легендарный';
    if (val >= 500)  return '💜 Редкий';
    if (val >= 100)  return '🔵 Необычный';
    return '⚪ Обычный';
}
...
function showGiftChoiceModal(gift, winAmount) {
    ...
    const sellPrice = Math.floor(winAmount * 0.5);

    const iconEl = $id('new-gift-icon');
    const nameEl = $id('new-gift-name');
    const tierEl = $id('new-gift-tier');
    const valEl  = $id('new-gift-value');
    const sellEl = $id('sell-amount');
    const keepEl = $id('keep-coins-label');

    if (iconEl) iconEl.textContent = emoji;
    if (nameEl) nameEl.textContent = gift.name;
    if (tierEl) tierEl.textContent = getGiftTier(winAmount);
    if (valEl)  valEl.textContent  = winAmount;
    if (sellEl) sellEl.textContent = sellPrice;
    if (keepEl) keepEl.textContent = `(+${winAmount} 🟡 в инвентарь)`;

    const modal = $id('new-gift-modal');
...
function sellGift() {
    ...
    closeNewGiftModal();
    currentNewGift = null;
    showNotif(`💰 Продано за ${sellPrice} 🟡`, '#22c55e');
}
...
function showUsdtPayScreen(inv) {
    ...
    body.innerHTML = `
        <div class="usdt-pay-screen">
            ...
            <div class="usdt-pay-wallet-label">На адрес (TRC20):</div>
            <div class="usdt-pay-wallet" onclick="copyUsdtWallet('${inv.wallet}')">${inv.wallet} <span class="usdt-copy-hint">📋</span></div>
...
// ===== ИНВЕНТАРЬ BOTTOM SHEET =====
let _invTab = 'active';

function openInventorySheet() {
    _invTab = 'active';
    const m = document.getElementById('inventory-sheet-modal');
...
function switchInvTab(tab) {
    _invTab = tab;
    const tA = document.getElementById('inv-tab-active');
    const tS = document.getElementById('inv-tab-sold');
    const activeStyle = 'flex:1;padding:9px;border-radius:10px;border:1.5px solid #7b5cff;background:rgba(123,92,255,0.15);color:#fff;font-size:0.8rem;font-weight:700;cursor:pointer;';
    const inactStyle  = 'flex:1;padding:9px;border-radius:10px;border:1.5px solid #2a2a3a;background:#1a1a2a;color:#aaa;font-size:0.8rem;font-weight:700;cursor:pointer;';
    if (tA) tA.style.cssText = tab === 'active' ? activeStyle : inactStyle;
    if (tS) tS.style.cssText = tab === 'sold'   ? activeStyle : inactStyle;
    renderInvGrid();
}
...
function renderInvGrid() {
    ...
    if (!grid) return;
    const inv = (userData.inventory || []).filter(g => _invTab === 'active' ? g.status !== 'sold' : g.status === 'sold');
    grid.innerHTML = '';
    if (!inv.length) {
        grid.innerHTML = '<div style="grid-column:span 3;text-align:center;color:#555;padding:32px 0;font-size:0.9rem;">Пусто</div>';
        return;
    }
    inv.forEach(g => {
        const emoji = (typeof GIFT_EMOJIS !== 'undefined' ? GIFT_EMOJIS[g.type] : null) || '🎁';
        const sellP = Math.floor((g.value || 0) * 0.5);
        const card = document.createElement('div');
        card.style.cssText = 'background:rgba(255,255,255,0.05);border:1.5px solid rgba(123,92,255,0.25);border-radius:14px;padding:12px 8px;text-align:center;cursor:pointer;transition:border-color 0.2s;';
        card.innerHTML = `
            <div style="font-size:2.2rem;margin-bottom:6px;">${emoji}</div>
            <div style="font-size:0.75rem;font-weight:800;color:#fff;margin-bottom:2px;">${g.name || 'Подарок'}</div>
            <div style="font-size:0.7rem;color:#fcd34d;font-weight:700;">${g.value || 0} 🟡</div>
            ${g.status !== 'sold' ? `<div style="font-size:0.65rem;color:#a78bfa;margin-top:4px;">продать: ${sellP} 🟡</div>` : '<div style="font-size:0.65rem;color:#555;margin-top:4px;">продано</div>'}
        `;
        if (g.status !== 'sold') {
            card.onclick = () => confirmSellFromInventory(g);
        }
        grid.appendChild(card);
    });
}

function confirmSellFromInventory(gift) {
    const sellP = Math.floor((gift.value || 0) * 0.5);
    if (!confirm(`Продать "${gift.name}" за ${sellP} 🟡?`)) return;
    gift.status = 'sold';
    userData.balance.silver += sellP;
    saveUserData();
    updateBalance();
    renderInvGrid();
    showNotif(`💰 Продано за ${sellP} 🟡`, '#22c55e');
}
